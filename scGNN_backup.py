...
from gae.model import GCNModelVAE, GATModelVAE, MultiHeadGAT
...
def GAEembedding(z, adj, args):
    ...
    model = GATModelVAE(feat_dim, args.GAEhidden1, args.GAEhidden2, 0)
    ...
    plt.savefig("train_roc_T0.5.png") 
    save_df = pd.DataFrame([list(range(1,201)), roc_list])
    save_df.to_csv("train_roc_T0.5.csv")
    return hidden_emb
...
if __name__ == "__main__":
    ...
    #             listResult, minMemberinCluster=args.minMemberinCluster, maxClusterNumber=30)
    ...
    #     adj, edgeList = generateAdj(zOut, para=args.knn_distance+':'+str(
    #         args.k), adjTag=True)
    #     print('---'+str(datetime.timedelta(seconds=int(time.time() -
